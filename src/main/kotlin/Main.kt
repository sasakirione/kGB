...
fun main() = application {
    ...
    Window(title = cpu.getGameTitle(), onCloseRequest = ::exitApplication, icon = iconImage) {
        MenuBar {
            Menu("ファイル", 'F') {
                Item("クイックセーブ", onClick = { cpu.save() }, shortcut = KeyShortcut(Key.S, ctrl = true))
                Item("クイックロード", onClick = {  }, shortcut = KeyShortcut(Key.L, ctrl = true))
            }
        }
